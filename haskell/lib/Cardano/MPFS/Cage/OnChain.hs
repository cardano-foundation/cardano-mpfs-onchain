...
-- |
-- Module      : Cardano.MPFS.Cage.OnChain
-- Description : On-chain type encodings for the cage validator
-- License     : Apache-2.0
--
-- Haskell types matching the Aiken on-chain datum\/redeemer
-- structures, their PlutusData encoding, and asset-name
-- derivation.
--
...
-- | Extract a 'ByteString' from a 'Data'
-- byte-literal.
...
instance UnsafeFromData CageDatum where
    unsafeFromBuiltinData bd = case unD bd of
        ...
        _ ->
            error
                "unsafeFromBuiltinData: CageDatum"
...
instance UnsafeFromData ProofStep where
    unsafeFromBuiltinData bd = case unD bd of
        ...
        _ ->
            error
                "unsafeFromBuiltinData: ProofStep"
...
deriveAssetName
    OnChainTxOutRef{txOutRefId, txOutRefIdx} =
        convert digest
      where
